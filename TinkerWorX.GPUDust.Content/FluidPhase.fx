	...
	in float2 texCoord : TEXCOORD0) : SV_Target0
{
	...
	if (compare(Direction, MOVE_DOWN))
	{
		...
		// ?X? -> ?O?
		// ?O?    ?X?
		if (compare(mc.x, FLUID) && compare(bc.x, NONE))
	...
	if (compare(Direction, MOVE_RIGHT)) // why isn't this MOVE_LEFT? Only MOVE_RIGHT works ...
	{
		...
		// OX? -> XO?
		// OX?    OX?
		if (compare(mc.x, FLUID) && compare(ml.x, NONE) && !compare(bc.x, NONE) && compare(bl.x, NONE))
			return float4(mc.xyz, MOVE_LEFT);

		// ???    ???
		// OXX -> XOX
		// XXX    XXX
		if (compare(ml.x, NONE) && compare(mc.x, FLUID) && !compare(bl.x, NONE) && !compare(bc.x, NONE) && !compare(br.x, NONE))
			return float4(mc.xyz, MOVE_LEFT);
	}
	...
	if (compare(Direction, MOVE_LEFT)) // why isn't this MOVE_RIGHT? Only MOVE_LEFT works ...
	{
		...
		// ?XO -> ?OX
		// ?XO    ?XO
		if (compare(mc.x, FLUID) && compare(mr.x, NONE) && !compare(bc.x, NONE) && compare(br.x, NONE))
			return float4(mc.xyz, MOVE_RIGHT);

		// ???    ???
		// XXO -> XOX
		// XXX    XXX
		if (compare(mr.x, NONE) && compare(mc.x, FLUID) && !compare(bl.x, NONE) && !compare(bc.x, NONE) && !compare(br.x, NONE))
			return float4(mc.xyz, MOVE_RIGHT);
	}
